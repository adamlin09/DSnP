...
{
   // TODO...
   CmdMap::const_iterator iter;  //print all helps of the commands in _cmdMap
...
CmdParser::parseCmd(string& option)
{
   ...
   CmdExec* e = 0;                           //get the command(first word) to check if it exists
   size_t pos = str.find(" "), pos_new;
   string cmd = str.substr(0, pos);
   e = getCmd(cmd);

   if (!e) {      //command not found
      cerr << "Illegal command!! \"" << cmd << "\"" << endl; 
   }
   else {
      pos_new = str.find_first_not_of(" ", pos);
      if (pos_new != string::npos)  option = str.substr(pos_new);
   }
   return e;
}
...
CmdParser::getCmd(string cmd)
{
   CmdExec* e = 0;
   // TODO...
   string tmp_cmd = cmd; size_t cmd_len = cmd.size();    //transform to upper letters to check
   for (size_t i = 0; i < cmd_len; ++i) {
      tmp_cmd[i] = toupper(cmd[i]);
   }
   CmdMap::iterator iter;
   for (iter = _cmdMap.begin(); iter != _cmdMap.end(); iter++) {
      size_t i_len = iter->first.size();
      string s = iter->first;
      if (cmd_len > i_len) {
         s = s + iter->second->getOptCmd();
         s.resize(cmd_len);
      }
      size_t s_len = s.size();
