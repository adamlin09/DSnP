...
//如果t不能被SIZE_T整除，就取t/SIZE_T的整數部份+1再乘以SIZE_T成為新的t，不然就是原本的t
//
...
//如果t不能被SIZE_T整除，就取t/SIZE_T的整數部份再乘以SIZE_T成為新的t，不然就是原本的t
...
class MemBlock
{
   ...
   bool getMem(size_t t, T*& ret) {
      // TODO
      t = toSizeT(t);
      if (getRemainSize() < t) {//如果current block裡剩下的空間比t小
         return false;
      }
      T* tmp = (T*)_ptr;
      ret = tmp;//把_ptr強制轉成T的指標後變成ret的分身
      _ptr += t;//讓_ptr往後移t
      return true;
   }
...
class MemRecycleList
{
   ...
   T* popFront() {
      // TODO
      T* tmp = _first;//先用tmp記錄原本的_first
      _first = (T*) *(size_t*)_first;//把_first這個指標強制轉成size_t的指標，取它的值(指到下一個element的位址)，再轉成T的指標後變成新的_first
      return tmp;//return原本的_first
   }
   // push the element 'p' to the beginning of the recycle list
   void  pushFront(T* p) {
      // TODO
      *(size_t*)p = (size_t)_first;//把_first這個指標強制轉成數字，變成p指到下一個element的位址
      _first = p;//讓_first變成p
   }
...
public:
   ...
   void reset(size_t b = 0) {
      ...
      while (_activeBlock->getNextBlock() != 0) {
         ...
         // cout << _recycleList[0].numElm() << endl;
      ...
      if (b != _blockSize) {
         if (b != 0) {
            MemBlock<T>* tmp = _activeBlock;
            _activeBlock = new MemBlock<T>(_activeBlock->getNextBlock(), b);
            delete tmp;
            _blockSize = b;
         }
      }
   ...
   T* getMem(size_t t) {
      ...
      // TODO
         ...
         if (!_activeBlock->getMem(t, ret)) {
            ...
            MemBlock<T>* tmp = _activeBlock;
            _activeBlock = new MemBlock<T>(tmp, _blockSize);
